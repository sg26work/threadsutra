...
for (const size of [20, 50, 100, 200]) {
  ...
  assert.equal(response.status, 200); assert.equal(json.rows, size); assert.ok(json.gridModel.length <= size); assert.deepEqual(json.zoneDTOList, json.gridModel); for (const key of ['rows', 'total', 'page', 'records']) assert.ok(key in json);
}
...
const invalidCharacters = await send('POST', { action: 'save', zoneCode: 'BAD-ZONE', locCode: 'LOC-DEL' }); assert.equal(invalidCharacters.response.status, 400); assert.equal(invalidCharacters.json.error, 'Please enter AlphaNumerics only');
const created = await send('POST', { action: 'save', zoneCode: apiZoneCode, descr: 'API Zone', status: true, locCode: 'LOC-DEL', zoneType: 'Normal', orderTypes: ['14'] }); assert.equal(created.response.status, 201);
...
const updated = await send('PUT', { action: 'save', id: created.json.row.id, zoneCode: apiZoneCode, descr: 'API Zone Updated', status: true, locCode: 'LOC-DEL', zoneType: 'Normal', orderTypes: ['14', '1'] }); assert.equal(updated.response.status, 200); assert.equal(updated.json.row.descr, 'API Zone Updated');
...
try {
  ...
  const initialSearch = page.waitForRequest((request) => request.url().endsWith('/api/wms-zones') && request.method() === 'POST');
  await page.goto(`${base}/app/wms/zone`); await page.getByRole('button', { name: 'Search', exact: true }).waitFor();
  const initialPayload = (await initialSearch).postDataJSON(); assert.equal(initialPayload.rows, 20); assert.equal(initialPayload.page, 1); assert.equal(initialPayload.locCode, 'LOC-DEL'); assert.equal(initialPayload.status, '1'); assert.equal(initialPayload.REQ_SEARCH_FLAG, true);
  assert.equal(await page.getByLabel('Site Location').isDisabled(), true);
  ...
  await page.getByLabel('Zone Code editor').fill('BAD-ZONE'); await page.getByRole('button', { name: 'Save', exact: true }).click(); await page.getByText('Please enter AlphaNumerics only', { exact: true }).waitFor();
  ...
  await page.locator('thead tr').nth(1).locator('input').first().fill(uiZoneCode); await page.getByRole('button', { name: 'Search', exact: true }).click(); await page.getByText('UI Zone Updated', { exact: true }).waitFor(); const resetRequest = page.waitForRequest((request) => request.url().endsWith('/api/wms-zones') && request.method() === 'POST'); await page.getByRole('button', { name: 'Reset', exact: true }).click(); const resetPayload = (await resetRequest).postDataJSON(); assert.equal(resetPayload.zoneCode, ''); assert.equal(resetPayload.locCode, 'LOC-DEL'); assert.equal(resetPayload.status, '1');
