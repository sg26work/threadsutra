...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const b = req.body; if (!text(b.channel_code) || !text(b.sku_code) || !text(b.channel_sku_code)) return res.status(400).json({ error: 'Channel, Eretail SKU and Channel SKU are required.' });
      ...
      return res.status(201).json(await insert('sku_channel_links', { ...b, channel_price: Number(b.channel_price || 0), back_order_qty: Number(b.back_order_qty || 0), maximum_sku_qty: Number(b.maximum_sku_qty || 0), status: b.status || 'Active', updated_by: 'super admin', updated_date: new Date().toISOString() }));
    }
