...
const normalize=row=>({...row,lotValCode:row.lot_val_code||row.code||'',description:row.description||row.name||'',isDefault:!!row.is_default,lottables:Array.from({length:7},(_,i)=>row.lottables?.[i]||{label:labels[i],mandatory:false,outward:false,mask:'',length:[0,4,5].includes(i)?'50':''})});
const validate=body=>{
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&(body.REQ_SEARCH_FLAG||body.action==='search')){
   ...
   return res.json({gridModel,rows:gridModel,page,records,total});
  }
